...
def predict():
    try:
        ...
        train['CU_M_ma'] = train['CU_M'].rolling(window=window_size).mean().fillna(train['CU_M'])  # Apply moving average

        # Log transformation for CU_M (after applying moving average)
        train['CU_M_log'] = np.log1p(train['CU_M_ma'])  # log1p for handling zeroes
