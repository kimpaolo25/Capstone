...
def predict():
    try:
        ...
        # **New: Add day of month feature before resampling**
        ...
        # Use the day_of_month feature in SARIMAX as an exogenous variable
        exog_train = df_monthly[['day_of_month']].dropna()

        # Data summary
        df_summary = df_monthly.describe()
        print("Data summary after feature engineering:", df_summary)

        # ---- SARIMAX for Amount_log with exog ----
        best_order_amount = find_best_sarimax_order(df_monthly['Amount_log'].dropna())
        model_amount = SARIMAX(df_monthly['Amount_log'].dropna(), 
                               order=best_order_amount[:3], 
                               seasonal_order=best_order_amount[3:] + (12,),
                               exog=exog_train)
        model_amount_fit = model_amount.fit(disp=False)

        # Forecast for the next 12 months using the day of the month exogenous variable
        forecast_steps = 12
        exog_forecast = pd.DataFrame({'day_of_month': [(i % 30) + 1 for i in range(forecast_steps)]})
        forecast_amount_log = model_amount_fit.get_forecast(steps=forecast_steps, exog=exog_forecast)
        forecast_amount_log_mean = forecast_amount_log.predicted_mean

        # Revert the log transformation to get the original scale forecast for 'Amount'
        forecast_amount = np.exp(forecast_amount_log_mean)

        # ---- SARIMAX for CU_M_log with exog ----
        ...
        forecast_cum_log = model_cum_fit.get_forecast(steps=forecast_steps, exog=exog_forecast)
        ...
        forecast_amount_aligned = forecast_amount_log_mean[:len(historical_amounts[-forecast_steps:])]
