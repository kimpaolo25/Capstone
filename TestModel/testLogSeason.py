...
def predict():
    try:
        ...
        # Seasonal differencing (assuming seasonality is 12 months for monthly data)
        df_monthly['Log_Amount_seasonal_diff'] = df_monthly['Log_Amount'].diff(12).dropna()
        df_monthly['Log_CU_M_seasonal_diff'] = df_monthly['Log_CU_M'].diff(12).dropna()
        ...
        # ---- ARIMA for Log_Amount_seasonal_diff ----
        model_auto_arima_amount = auto_arima(train['Log_Amount_seasonal_diff'].dropna(), seasonal=False, stepwise=True, trace=True)
        best_order_amount = model_auto_arima_amount.order
        model_amount = ARIMA(train['Log_Amount_seasonal_diff'].dropna(), order=best_order_amount)
        ...
        # Revert the seasonal differencing to get the original scale forecast for 'Amount'
        forecast_amount_seasonal_diff_reverted = forecast_amount_diff_mean + df_monthly['Log_Amount'].iloc[train_size - 12]

        # Exponentiate to revert the log transformation
        forecast_amount = np.exp(forecast_amount_seasonal_diff_reverted)

        # ---- ARIMA for Log_CU_M_seasonal_diff ----
        model_auto_arima_cum = auto_arima(train['Log_CU_M_seasonal_diff'].dropna(), seasonal=False, stepwise=True, trace=True)
        best_order_cum = model_auto_arima_cum.order
        model_cum = ARIMA(train['Log_CU_M_seasonal_diff'].dropna(), order=best_order_cum)
        ...
        # Revert the seasonal differencing to get the original scale forecast for 'CU_M'
        forecast_cum_seasonal_diff_reverted = forecast_cum_diff_mean + df_monthly['Log_CU_M'].iloc[train_size - 12]
        
        # Exponentiate to revert the log transformation
        forecast_cum = np.exp(forecast_cum_seasonal_diff_reverted)
