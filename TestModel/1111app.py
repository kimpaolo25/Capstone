...
def predict():
    try:
        ...
        # ---- SARIMAX for Amount_log with fixed order ----
        order_amount = (1, 0, 0)  # Fixed non-seasonal order
        ...
        forecast_steps = 12
        forecast_amount_log = model_amount_fit.get_forecast(steps=forecast_steps)
        forecast_amount_log_mean = forecast_amount_log.predicted_mean

        # Reverse the log transformation for Amount
        forecast_amount = np.expm1(forecast_amount_log_mean)

        # ---- Log Transformation for CU_M ----
        df_monthly['CU_M_log'] = np.log1p(df_monthly['CU_M'])  # log1p for handling zeroes

        # Apply first differencing for CU_M_log
        ma_diff = df_monthly['CU_M_log'].diff().dropna()

        # Fit SARIMAX model for CU_M with fixed order
        order_cum = (1, 0, 2)  # Adjust orders as necessary
        seasonal_order_cum = (1, 0, 0, 12)  # Fixed seasonal order
        model_cum = SARIMAX(ma_diff, order=order_cum,
        ...
        last_cu_m_log = df_monthly['CU_M_log'].iloc[-1]  # Last value of the log-transformed CU_M
