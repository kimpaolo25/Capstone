...
def predict():
    try:
        ...
        for _ in range(100):  # Number of random searches
            p = random.randint(0, 3)  # AR term
            d = random.randint(0, 1)  # Differencing
            q = random.randint(0, 3)  # MA term
            try:
                model_amount = ARIMA(train['Log_Amount'].dropna(), order=(p, d, q))
                model_amount_fit = model_amount.fit()
                mse = mean_squared_error(train['Log_Amount'].dropna(), model_amount_fit.fittedvalues)
        ...
        print("Best ARIMA order:", best_order_amount)

        # Fit best ARIMA model
        model_amount = ARIMA(train['Log_Amount'].dropna(), order=best_order_amount)
        model_amount_fit = model_amount.fit()

        # ---- Random Search for Exponential Smoothing ----
        best_mse_exp = float('inf')
        best_trend = None
        best_seasonal = None
        ...
        for _ in range(100):  # Number of random searches
            ...
            try:
                model_exp_smoothing_amount = ExponentialSmoothing(train['Log_Amount'].dropna(), trend=trend, seasonal=seasonal, seasonal_periods=12)
                model_exp_fit_amount = model_exp_smoothing_amount.fit()
                mse = mean_squared_error(train['Log_Amount'].dropna(), model_exp_fit_amount.fittedvalues)
        ...
        print("Best Exponential Smoothing trend:", best_trend, "seasonal:", best_seasonal)

        # Fit best Exponential Smoothing model
        model_exp_smoothing_amount = ExponentialSmoothing(train['Log_Amount'].dropna(), trend=best_trend, seasonal=best_seasonal, seasonal_periods=12)
        model_exp_fit_amount = model_exp_smoothing_amount.fit()
        ...
        for _ in range(100):
            ...
            try:
                model_cum = ARIMA(train['Log_CU_M'].dropna(), order=(p, d, q))
                model_cum_fit = model_cum.fit()
                mse = mean_squared_error(train['Log_CU_M'].dropna(), model_cum_fit.fittedvalues)
        ...
        print("Best CU_M ARIMA order:", best_order_cum)

        model_cum = ARIMA(train['Log_CU_M'].dropna(), order=best_order_cum)
        model_cum_fit = model_cum.fit()

        best_mse_exp_cum = float('inf')
        best_trend_cum = None
        best_seasonal_cum = None

        for _ in range(100):
            trend = random.choice(trend_options)
            seasonal = random.choice(seasonal_options)
            try:
                model_exp_smoothing_cum = ExponentialSmoothing(train['Log_CU_M'].dropna(), trend=trend, seasonal=seasonal, seasonal_periods=12)
                model_exp_fit_cum = model_exp_smoothing_cum.fit()
                mse = mean_squared_error(train['Log_CU_M'].dropna(), model_exp_fit_cum.fittedvalues)

                if mse < best_mse_exp_cum:
                    best_mse_exp_cum = mse
                    best_trend_cum = trend
                    best_seasonal_cum = seasonal

            except Exception as e:
                continue

        print("Best CU_M Exponential Smoothing trend:", best_trend_cum, "seasonal:", best_seasonal_cum)

        model_exp_smoothing_cum = ExponentialSmoothing(train['Log_CU_M'].dropna(), trend=best_trend_cum, seasonal=best_seasonal_cum, seasonal_periods=12)
        model_exp_fit_cum = model_exp_smoothing_cum.fit()

        # Forecast on the test data length for CU_M
