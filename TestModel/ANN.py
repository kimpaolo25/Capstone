...
def predict():
    try:
        ...
        mape_amount = np.mean(np.abs((y_test_amount - forecast_amount) / y_test_amount)) * 100 if not np.all(y_test_amount == 0) else np.inf

        # Calculate MAPE for CU_M
        mape_cum = np.mean(np.abs((y_test_cum - forecast_cum) / y_test_cum)) * 100 if not np.all(y_test_cum == 0) else np.inf

        # Print the accuracy metrics
        print(f"ANN Model for Amount - MSE: {mse_amount}, RMSE: {rmse_amount}, MAE: {mae_amount}, MAPE: {mape_amount}")
        print(f"ANN Model for CU_M - MSE: {mse_cum}, RMSE: {rmse_cum}, MAE: {mae_cum}, MAPE: {mape_cum}")
