...
def predict():
    try:
        ...
        # Apply second differencing to 'Amount' and 'CU_M'
        df_monthly['Amount_diff1'] = df_monthly['Amount'].diff().dropna()
        df_monthly['CU_M_diff1'] = df_monthly['CU_M'].diff().dropna()
        df_monthly['Amount_diff2'] = df_monthly['Amount_diff1'].diff().dropna()
        df_monthly['CU_M_diff2'] = df_monthly['CU_M_diff1'].diff().dropna()
        ...
        # ---- ARIMA for Amount_diff2 ----
        # Fit auto_arima model to find the best order for 'Amount_diff2'
        model_auto_arima_amount = auto_arima(
            train['Amount_diff2'].dropna(),
            seasonal=False,  # No seasonal component in ARIMA
            stepwise=True,
            trace=True
        )

        # Print the best order found by auto_arima for 'Amount_diff2'
        print(f"Best order found for Amount_diff2: {model_auto_arima_amount.order}")

        # Use the best order found by auto_arima
        best_order_amount = model_auto_arima_amount.order

        # Fit ARIMA model for 'Amount_diff2'
        model_amount = ARIMA(
            train['Amount_diff2'].dropna(),
        ...
        # Create a forecast for 'Amount_diff2'
        forecast_steps = len(test)
        forecast_amount_diff2 = model_amount_fit.get_forecast(steps=forecast_steps)
        forecast_amount_diff2_mean = forecast_amount_diff2.predicted_mean

        # Revert the second differencing to get the original scale forecast for Amount
        forecast_amount_diff1 = forecast_amount_diff2_mean.cumsum() + df_monthly['Amount_diff1'].iloc[-len(forecast_amount_diff2_mean):].values
        forecast_amount = forecast_amount_diff1.cumsum() + df_monthly['Amount'].iloc[-len(forecast_amount_diff1):].values

        future_dates_amount = [test.index[i] for i in range(forecast_steps)]

        # ---- ARIMA for CU_M_diff2 ----
        # Fit auto_arima model to find the best order for 'CU_M_diff2'
        model_auto_arima_cum = auto_arima(
            train['CU_M_diff2'].dropna(),
            seasonal=False,  # No seasonal component in ARIMA
            stepwise=True,
            trace=True
        )

        # Print the best order found by auto_arima for 'CU_M_diff2'
        print(f"Best order found for CU_M_diff2: {model_auto_arima_cum.order}")

        # Use the best order found by auto_arima
        best_order_cum = model_auto_arima_cum.order

        # Fit ARIMA model for 'CU_M_diff2'
        model_cum = ARIMA(
            train['CU_M_diff2'].dropna(),
        ...
        # Create a forecast for 'CU_M_diff2'
        forecast_cum_diff2 = model_cum_fit.get_forecast(steps=forecast_steps)
        forecast_cum_diff2_mean = forecast_cum_diff2.predicted_mean

        # Revert the second differencing to get the original scale forecast for CU_M
        forecast_cum_diff1 = forecast_cum_diff2_mean.cumsum() + df_monthly['CU_M_diff1'].iloc[-len(forecast_cum_diff2_mean):].values
        forecast_cum = forecast_cum_diff1.cumsum() + df_monthly['CU_M'].iloc[-len(forecast_cum_diff1):].values
