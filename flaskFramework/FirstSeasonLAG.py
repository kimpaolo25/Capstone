...
def predict():
    try:
        ...
        seasonal_period = 12  # For monthly data, it's 12
        df_monthly['Amount_seasonal_diff'] = df_monthly['Amount'] - df_monthly['Amount'].shift(seasonal_period)
        df_monthly['CU_M_seasonal_diff'] = df_monthly['CU_M'] - df_monthly['CU_M'].shift(seasonal_period)

        df_monthly.dropna(inplace=True)  # Drop NA values resulting from differencing

        # Apply first differencing to the seasonally differenced series
        df_monthly['Amount_diff'] = df_monthly['Amount_seasonal_diff'].diff().dropna()
        df_monthly['CU_M_diff'] = df_monthly['CU_M_seasonal_diff'].diff().dropna()
        ...
        model_auto_arima_amount = auto_arima(
            ...
            seasonal=False,  # Seasonal component is handled earlier
            stepwise=True,
            trace=True
        )
        ...
        # Fit ARIMA model for 'Amount_diff'
        model_amount = ARIMA(
            train['Amount_diff'].dropna(),
        ...
        # Revert first differencing
        forecast_amount_seasonal_diff = forecast_amount_diff_mean.cumsum() + df_monthly['Amount_seasonal_diff'].iloc[-len(forecast_amount_diff_mean):].values
        forecast_amount = forecast_amount_seasonal_diff.cumsum() + df_monthly['Amount'].iloc[-len(forecast_amount_seasonal_diff):].values
        ...
        # Revert first differencing
        forecast_cum_seasonal_diff = forecast_cum_diff_mean.cumsum() + df_monthly['CU_M_seasonal_diff'].iloc[-len(forecast_cum_diff_mean):].values
        forecast_cum = forecast_cum_seasonal_diff.cumsum() + df_monthly['CU_M'].iloc[-len(forecast_cum_seasonal_diff):].values
